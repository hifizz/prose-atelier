...
`export default`. The runtime renderer (`app/_mdx/renderer.tsx`) handles the
shell, plugins, and component registration in one place.
...
```ts
import { ArticleLayout } from "@/app/_mdx";
...
To customize anything: see `app/_mdx/README.md`. The Shiki config (themes,
transformers, language aliases) lives in `_mdx/rehype-shiki.mjs` as plain
JS data — change one file, all articles update.

---

That's it. Drop a `.mdx` in `content/` and you get the same shell. To copy
this whole setup to a fresh Next.js project, see `app/_mdx/README.md` —
about 30 seconds of work.
